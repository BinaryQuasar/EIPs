...
4. **`Provider#isEnabled`**
    ...
    Providers exposed by DOM environments define a new `isEnabled` property that is set to `true` if the user approved full provider access or `false` if the user denies full provider access.
